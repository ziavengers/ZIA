...
void contextEmit(const std::string& context, const std::string& name @params@)
{
  ...
  if (itm != _slots.end())
	...
	if (it->context == "*" || it->context == context)
	  {
	    ...
	    if (caller)
	      {
		@setparams@
		  zia::utils::Singleton< zia::core::ThreadPool >::instance()->push(this, ito->first, context, it->c);
	      }
	    else
	      {
